// Controller Caddy — v010
// v009 + a 4th remote slot: a 29x41 rounded pocket for an HDMI switcher remote,
// added as a new MIDDLE column between the remote column and the dogbone (TV
// remote) hole. The dogbone shifts to the right column. The HDMI slot spans
// all three rows so it's centered front-to-back, level with the Apple remote.
//
...
/* [Per-hole layout] */
cells = [
    // col 1: three remote slots | col 2: HDMI switcher (29x41, spans 3 rows)
    //        | col 3: DualSense/TV dogbone slot (44mm wells, 168mm, spans 3 rows)
    // cell format: [shape, sx, sy, depth, span_cols, span_rows, corner]
    [ ["s", 34, 10],                          ["s", 29, 41, undef, 1, 3],  ["p", 44, 168, 55, 1, 3] ],
