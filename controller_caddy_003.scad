...
col_gaps = [20];     // 20mm between col 1 and col 2
row_gaps = [];
wall    = 4;
floor_h = 4;
body_corner = 10;
...
function cx(r, c) = wall + sum([for (i = [0:1:c-1]) col_w[i] + wall + gapx(i)])
                    + span_w(c, spc(r,c)) / 2;
function cy(r, c) = wall + sum([for (i = [0:1:r-1]) row_d[i] + wall + gapy(i)])
                    + span_d(r, spr(r,c)) / 2;

W = sum(col_w) + (n_cols + 1) * wall + sum([for (i = [0:1:n_cols-2]) gapx(i)]);
D = sum(row_d) + (n_rows + 1) * wall + sum([for (i = [0:1:n_rows-2]) gapy(i)]);
