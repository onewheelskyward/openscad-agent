// Coffee Table Caddy — v018
// v017, but the toothpaste sidecar is moved to the RIGHT wall (opposite the
// toothbrush side) by mirroring it across the body centerline. Same 60mm low
// shelf with lip + domed bumps; a tube lays horizontally, front-to-back.
//
...
sidecar_w       = 70;   // how far the shelf extends out from the left wall (mm)
...
sc_bump_pitch   = 11;   // bump grid spacing
...
// Toothpaste sidecar: low rounded shelf hanging off the RIGHT wall (opposite
// the toothbrush side), with a raised lip rim and a grid of domed bumps.
// Built on the left, then mirrored across the body centerline to the right.
module toothpaste_sidecar() {
    ...
    translate([W, 0, 0]) mirror([1, 0, 0]) {
        translate([-sidecar_w, 0, 0]) {
            ...
            // lip rim around the perimeter (inner part is buried in the wall)
            difference() {
        ...
        // domed bumps on the exposed deck
