// Coffee Table Caddy — v017
// v016 + a toothpaste "sidecar": a low rounded shelf extending 60mm off the
// toothbrush (left) side, matching the body corner radius, with a raised lip
// around the edge and a grid of domed bumps on the deck (instead of the
// cross-hatch drain holes). A toothpaste tube lays horizontally in it,
// running front-to-back. See the [Toothpaste sidecar] params.
//
...
cells = [
    ...
    [ ["r", 35, 35, 35, 1, 1, -18, 0], ["s", 29, 31, 35, 1, 1, 19.33, 0] ]
];
...
/* [Toothpaste sidecar] */
sidecar         = true; // add the toothpaste shelf on the toothbrush (left) side
sidecar_w       = 60;   // how far the shelf extends out from the left wall (mm)
sidecar_h       = 8;    // deck height (low platform)
sidecar_overlap = 10;   // how far the shelf merges into the body's left wall
sc_lip_w        = 3;    // lip thickness
sc_lip_h        = 3;    // lip height above the deck
sc_bump_r       = 3.5;  // bump radius (footprint)
sc_bump_squash  = 0.55; // bump dome flattening (z scale)
sc_bump_pitch   = 14;   // bump grid spacing
...
// Toothpaste sidecar: low rounded shelf hanging off the left (toothbrush)
// wall, with a raised lip rim and a grid of domed bumps on the deck.
module toothpaste_sidecar() {
    sw = sidecar_w + sidecar_overlap;   // total slab width in x
    deck_top = sidecar_h;
    translate([-sidecar_w, 0, 0]) {
        // deck
        rounded_slab(sw, D, sidecar_h, body_corner);
        // lip rim around the perimeter (inner part is buried in the body wall)
        difference() {
            rounded_slab(sw, D, deck_top + sc_lip_h, body_corner);
            translate([sc_lip_w, sc_lip_w, deck_top])
                rounded_slab(sw - 2*sc_lip_w, D - 2*sc_lip_w,
                             sc_lip_h + 1, max(body_corner - sc_lip_w, 1));
        }
    }
    // domed bumps on the exposed deck (left of the body, x < 0)
    for (bx = [-sidecar_w + sc_lip_w + 4 : sc_bump_pitch : -6],
         by = [sc_lip_w + 6 : sc_bump_pitch : D - sc_lip_w - 6])
        translate([bx, by, deck_top])
            scale([1, 1, sc_bump_squash]) sphere(r = sc_bump_r);
}
