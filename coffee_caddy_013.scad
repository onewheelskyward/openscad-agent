// Coffee Table Caddy — v013
// v012 + 20mm wider (148 -> 168mm). Column widths are locked to the pocket
// sizes, so the extra width is added as horizontal spacing: width_pad is
// spread evenly across the three vertical gaps (left border | center channel
// | right border) via hgap, keeping the layout balanced and roughly matching
// the row gaps. Depth is unchanged.
//
...
back_gap = ((W - 2*hgap) - csx(3,0) - floss_l - csx(3,1)) / 4;
floss_cx = hgap + back_gap + csx(3,0) + back_gap + floss_l/2;
floss_cy = cy(3,0);
