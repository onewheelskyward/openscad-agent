// Coffee Table Caddy — v011
// v010, but the three back-row holes (round pill | floss | rectangle) are
// evenly spaced: the four gaps wall->round->floss->rect->wall are all equal.
// The rectangle's push and the floss x-position are both derived from that
// equal gap (see back_gap below). Both diagonal round holes are kept.
//
...
cells = [
    ...
    [ ["r", 35, 35, 35, 1, 1, -12, 0], ["s", 29, 31, 35, 1, 1, 9, 0] ]
];
...
back_gap = ((W - 2*wall) - csx(3,0) - floss_l - csx(3,1)) / 4;
floss_cx = wall + back_gap + csx(3,0) + back_gap + floss_l/2;
floss_cy = cy(3,0);
