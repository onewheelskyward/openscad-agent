// Coffee Table Caddy — v008
// v007 + 20mm extra length (new front strip), diagonal offset on the two
// round holes, and a dental-floss slot cut into the new front strip.
//
...
/* [Front strip] */
extra_length = 20;   // extra mm of overall length added at the front
floss_w      = 12;   // dental-floss slot width (y, front-to-back)
floss_depth  = 10;   // floss slot depth
        ...
        if (used(r, c))
            translate([cx(r, c), cy(r, c), H - cdp(r, c)])
                pocket(csh(r,c), csx(r,c), csy(r,c), cdp(r,c),
                       H - cdp(r, c));
    translate([W/2, extra_length/2, H - floss_depth])
