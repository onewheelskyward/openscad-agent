// Controller Caddy — v015
// v014 with two fit tweaks: the Apple remote slot grows 1mm each direction
// (36x10 -> 37x11) for an easier fit, and the HDMI switcher slot shrinks 4mm
// front-to-back (41x29 -> 41x25).
//
// v013, but the HDD platter holder uses a new shape "c" (disc cradle): a
// horizontal cylindrical groove (radius = disc radius) cut across the slot
// width, so the round platters nest along their curved edge instead of
// bottoming out on a flat floor. This pocket has no drain/vent lattice.
// For "c": size_x = groove width (stack thickness), size_y = disc diameter,
//          depth = how deep the groove cuts below the top surface.
//
// v012, but the 4 remote slots in the left column are now evenly spaced:
// instead of centering each in its (dogbone-sized) row, the column distributes
// the slots down the usable depth with equal edge-to-edge gaps (remote_cy()).
//
// v010, reworked: the HDMI switcher (29x41) moves INTO the left remote stack,
// behind the Apple remote, so the left column is now 4 slots (thin / Apple /
// HDMI / latticed). The middle column instead holds a hard-drive-platter
// cradle: a narrow on-edge slot (22 wide x 90 long x 32 deep) that holds a
// stack of 3.75" (95mm) circular platters standing vertically. The dogbone
// (PlayStation controller) moves to the right column, spanning all 4 rows.
//
...
/* [Per-hole layout] */
cells = [
    // col 1: remote stack (4 slots) | col 2: HDD platter cradle (22x90, on-edge,
    //        spans 4 rows) | col 3: DualSense dogbone (44mm wells, 168, spans 4)
    // cell format: [shape, sx, sy, depth, span_cols, span_rows, corner]
    [ ["s", 34, 10],                          ["c", 18, 95, 32, 1, 4],  ["p", 44, 168, 55, 1, 4] ],
    [ ["s", 37, 11, undef, undef, undef, 2],  ["-"],                    ["-"] ],  // Apple remote: square corners
    [ ["s", 41, 25],                          ["-"],                    ["-"] ],  // HDMI switcher (rounded, rotated 90)
    [ ["s", 44, 33],                          ["-"],                    ["-"] ]   // wide latticed slot
];
...
// [after remotes -> HDMI, after HDMI -> dogbone]
col_gaps = [10, 10];
row_gaps = [];
...
function used(r, c)  = csh(r, c) == "s" || csh(r, c) == "r" || csh(r, c) == "p" || csh(r, c) == "c";
...
// Even vertical spacing for the remote column (col 0): rather than centering
// each remote in its (dogbone-sized) row, distribute the slots down the usable
// depth with equal edge-to-edge gaps. n_rows slots -> n_rows+1 equal gaps.
remote_span  = D - 2*wall - pad_f - pad_b;
function rdep(r) = used(r, 0) ? csy(r, 0) : 0;
remote_total = sum([for (r = [0:n_rows-1]) rdep(r)]);
remote_gap   = (remote_span - remote_total) / (n_rows + 1);
function remote_cy(r) = wall + pad_f + remote_gap
                        + sum([for (i = [0:1:r-1]) rdep(i) + remote_gap])
                        + rdep(r) / 2;

echo(str("Outer size: ", W, " x ", D, " x ", H, " mm"));
echo(str("Remote gap (even): ", remote_gap, " mm"));
...
// disc cradle: a horizontal cylindrical groove (axis along x / the stack
// width) sized to the disc radius, so round platters nest along their edge.
// Built with its top opening at the origin plane, cutting downward.
module disc_cradle(width, disc_d, depth, clr = 0.5) {
    R = disc_d / 2 + clr;
    translate([0, 0, -depth + R])
        rotate([0, 90, 0])
            cylinder(h = width, r = R, center = true);
}
...
difference() {
    ...
    for (r = [0:n_rows-1], c = [0:n_cols-1])
        if (used(r, c)) {
            // col 0 (remotes) uses even spacing; spanning cols use row centers
            yc = c == 0 ? remote_cy(r) : cy(r, c);
            if (csh(r, c) == "c")
                translate([cx(r, c), yc, H])
                    disc_cradle(csx(r,c), csy(r,c), cdp_eff(r,c));
            else
                translate([cx(r, c), yc, H - cdp_eff(r, c)])
