// Coffee Table Caddy — v009
// v008, but the floss slot is no longer full-width: it's a single pocket
// sized to drop a Glide floss container into (standing on edge), tucked to
// the RIGHT end of the front strip (opposite the round-hole column).
//
...
cells = [
    ...
    // row 4: 35mm round, nudged -x (stagger vs. row 3 -> diagonal look) | 29x31 slot
    [ ["r", 35, 35, 35, 1, 1, -12, 0], ["s", 29, 31] ]
];
...
/* [Front strip] */
extra_length = 24;   // extra mm of overall length added at the front
// Floss slot sized for a Glide floss container (~52 x 40 x 17mm) standing
// on edge, plus ~2mm clearance all round.
...
floss_margin = 6;    // gap kept from the right body wall
...
// floss slot: right end of the front strip; front edge sits one wall in.
floss_cx = W - wall - floss_margin - floss_l / 2;
floss_cy = wall + floss_w / 2;
