...
//
// v3: riser chase widened 10 -> 18mm — the balance connector exits the
//     board's short (-X) edge, so the plug body + wire bend get ~20mm
//     of room beyond the board edge. Tray is now flush with frame width.
//
...
vent_w      = 142;   // reaches under the tray's riser window
vent_h      = 30;
...
tray_wall   = 2.5;
tray_floor  = 2.5;
riser_w     = 18;    // wire chase at -X end: balance plug + bend room
rib_t       = 2;     // rib separating riser from board
rib_h       = 6;
bay_l       = bms_l + 0.5 + rib_t + riser_w;   // 146.5
bay_w       = bms_w + 0.5;
tray_w      = bay_l + 2*tray_wall;             // 151.5
...
module tray() {
    difference() {
        ...
        translate([-bay_l/2 + 3, -12, -1])
            cube([riser_w - 4, 24, tray_floor + 2]);
