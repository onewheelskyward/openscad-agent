// Coffee Table Caddy — v014
// v013, but the floss now has a clean 5mm gap to each back-row neighbour:
// the round pill hole on the left and the 29x31 (toothbrush) slot on the
// right. floss_cx is set 5mm off the round's edge; the toothbrush slot's
// x-offset is set so its left edge sits 5mm past the floss. The leftover
// width falls into the right margin.
//
...
cells = [
    ...
    [ ["r", 35, 35, 35, 1, 1, -12, 0], ["s", 29, 31, 35, 1, 1, 5.33, 0] ]
];
...
// floss sits floss_gap (5mm) to the right of the round pill hole's edge.
// The 29x31 slot's +5.33 x-offset (in cells) puts its left edge 5mm past
// the floss, giving equal 5mm gaps on both sides.
floss_gap = 5;
