// Coffee Table Caddy — v010
// v009, but the floss slot moves into the BACK row, sitting between the
// back round pill-bottle hole and that row's rectangle. The rectangle is
// pushed toward the right wall to open a gap, giving three holes across
// the back row: round pill | floss | rectangle. Both diagonal round holes
// are kept. (Front strip is now an empty margin — the +20mm length stays.)
//
...
cells = [
    ...
    //        | 29x31 slot pushed +x toward the wall to make room for floss
    [ ["r", 35, 35, 35, 1, 1, -12, 0], ["s", 29, 31, 35, 1, 1, 10, 0] ]
];
...
/* [Front strip] */
extra_length = 20;   // extra mm of overall length (front margin)
...
function cy(r, c) = extra_length + wall + sum([for (i = [0:1:r-1]) row_d[i] + wall])
                    + span_d(r, spr(r,c)) / 2 + coy(r, c);

W = sum(col_w) + (n_cols + 1) * wall;
D = sum(row_d) + (n_rows + 1) * wall + extra_length;
...
// floss slot: back row, centered in the gap between the round pill hole
// (cell 3,0) and the rectangle (cell 3,1) after the rectangle is pushed over.
floss_cx = ((cx(3,0) + csx(3,0)/2) + (cx(3,1) - csx(3,1)/2)) / 2;
floss_cy = cy(3,0);
